(function () {
  ...
  // Сигнал для автотестов: все обработчики форм (логин, регистрация и т.д.) уже навешаны,
  // можно безопасно кликать по кнопкам — без этого маркера тест может кликнуть раньше, чем
  // app.js успеет выполниться (особенно при холодном старте на Render), и тогда сработает
  // нативный submit формы вместо перехвата через fetch — см. #login-form выше.
